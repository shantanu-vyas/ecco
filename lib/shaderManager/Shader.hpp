...
enum LoadType { LT_FromSource, LT_FromBinarySource };
...
public:
  ...
  Shader(const std::string &name, const std::string &filename,
         LoadType loadType, ShaderType shaderType);

  // Only ShaderManager and ShaderProgram should be able to call these
  [[nodiscard]] GLuint GetShaderID() const;
  // These are all marked nodiscard because we need to throw or something if
  // they come back bad OpenGL lets you operate with unloaded/noncompiled
  // shaders and it will lead to fuckyness
...
private:
  ...
  /* I need to think about who will call this. The user will modify a shader at
   * runtime and we need to relink all ShaderPrograms that are using it so this
   * should be called by a public reload func? in which case the protected
   * reloadfromfile and savecachedbinary files need to be public... figure this
   * out after writing the shaderprogram and writing the impl for this file */
